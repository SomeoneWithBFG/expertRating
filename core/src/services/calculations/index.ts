...
class CalculationsService implements ICalculationsService {
    pairComparsion (binaryMatrix: number[][], x: number, y: number) {
        ...
        for (let i = 0; i < numOfEl; i++) {
            for (let j = 0; j < numOfEl; j++) {
                ...
                result.values[i] += binaryMatrix[i][j];
            }
        }
        for (let i = 0; i < numOfEl; i++) {
        ...
        for (let i = 0; i < numOfEl - 2; i++) {
            if (rawEvaluations[i].weight <= (rawEvaluations[i + 1].weight + rawEvaluations[i + 2].weight)) {
                rawEvaluations[i].weight += 
                    ...
                    result.causedCorrections += rawEvaluations[i].index + " <=> (" + rawEvaluations[i+1].index + "&" + rawEvaluations[i+2].index + ")\n"
            }
        }
        for (let i = 0; i < numOfEl; i++) {
    ...
    preference (inputMatrix: number[][], x: number, y: number) {
        ...
        for (let i = 0; i < x; i++) {
            result.weights[i] = parseFloat((result.sumMarks[i] / result.sumOfMarks).toFixed(4));
        }
        let foo: number[] = []
        for (let i = 0; i < x; i++) {
            foo[i] = result.weights[i];
    ...
    kondorse (inputMatrix: number[][], x: number, y: number) {
            ...
            for (let j = 0; j < x; j++) {
                for (let k = j+1; k < x; k++) {
                    if (inputMatrix[i][j] < inputMatrix[i][k])
                        result.modMatrix[j][k]++;
                    else
                        result.modMatrix[k][j]++;
                }
            }
        }
        for (let i = 0; i < x; i++) {
        ...
        for (let i = 0; i < x; i++) {
            let res = 0;
            for (let j = 0; j < x; j++) {
                if (result.modMatrix[i][j] > result.modMatrix[j][i])
                    res++;
            }
            if (res === x-1)
                result.best = i + 1
