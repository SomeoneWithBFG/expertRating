<template>
  <div>
    <div class="text">
      ...
      >
        <a-select-option
          v-for="(val, index) in new Array(9)"
          :key="index"
          :value="index"
        >
          {{ index + 2 }}
...
export default {
  ...
  computed: {
    ...
    disableY: function () {
      if (this.isCalculated || this.method === 'pairComparsion') return true
      switch (this.method) {
        default:
          return false
      }
  ...
  methods: {
    ...
    handleChangeX(value) {
      if (this.method === 'pairComparsion') this.changeY(value + 2)
