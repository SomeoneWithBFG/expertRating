<template>
  <div class="pairComparsionDataContainer">
    <div>Цена каждой цели: {{ dataToShow.values }}</div>
    <div>Сумма цен: {{ dataToShow.sumOfValues }}</div>
    <div>Исковые веса целей: {{ dataToShow.weights }}</div>
  ...
  computed: {
    dataToShow: function () {
      ...
      this.solution.values.map((item, index) => {
        ...
        if (index + 1 !== this.solution.values.length) data.values += ' | '
      })
