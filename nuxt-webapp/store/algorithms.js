...
function pairComparsion(binaryMatrix, x, y) {
  ...
  for (var i = 0; i < numOfEl; i++) {
    for (var j = 0; j < numOfEl; j++) {
      if (binaryMatrix[i][j] >= 0) objToReturn.values[i] += binaryMatrix[i][j]
    }
  }
  for (var i = 0; i < numOfEl; i++) {
  ...
  for (var i = 0; i < numOfEl; i++) {
    objToReturn.weights[i] =
      (objToReturn.values[i] * 1.0) / objToReturn.sumOfValues
  }
...
function sequentiallyComparison(inputMatrix, x, y) {
  ...
  for (var i = 0; i < y; i++) {
    rawEvaluations.push({
      index: inputMatrix[i][0],
      weight: inputMatrix[i][1],
      resultWeight: 0,
    })
  }
  var objToReturn = {
...
function preference(inputArray, x, y) {
  ...
  for (var i = 0; i < x; i++) {
    objToReturn.weights[i] = (
      objToReturn.sumMarks[i] / objToReturn.sumOfMarks
    ).toFixed(4)
  }
...
function kondorse(inputArray, x, y) {
  ...
  for (var i = 0; i < y; i++) {
    for (var j = 0; j < x; j++) {
      for (var k = j + 1; k < x; k++) {
        if (inputArray[i][j] < inputArray[i][k]) objToReturn.modMatrix[j][k]++
        else objToReturn.modMatrix[k][j]++
      }
    }
  }
  for (var i = 0; i < x; i++) {
  ...
  for (var i = 0; i < x; i++) {
    var res = 0
    for (var j = 0; j < x; j++) {
      if (objToReturn.modMatrix[i][j] > objToReturn.modMatrix[j][i]) res++
    }
    if (res == x - 1) objToReturn.best = i + 1
  }
  return objToReturn
}
